...
impl Migrator {
    ...
    async fn generate_readme_markdown(
        ...
        let mut p = MarkdownStepPrinter::new(buf, "#");
        ...
        for k2p_conf_path in k2p_conf_paths.iter() {
            ...
            // current user.
            p.cmd(format!("kmip2pkcs11 -c {k2p_conf_path} --check-config"))?;
        }
        ...
        p.println("WARNING: Executing this command will SHUTDOWN your OpenDNSSEC instance.")?;
        ...
        p.println(format!("WARNING: This step will cause zones to be added and signed. If you have a lot of zones or very large zones this could use a lot of CPU and/or memory. Please review the commands in '{cmd_file_path}' before executing the script."))?;
...
impl MarkdownStepPrinter {
    fn new<T: Into<String>>(buf: String, heading: T) -> Self {
        Self {
            step_idx: 1,
            step_start: true,
            buf,
